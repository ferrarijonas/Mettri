 ...
 * CanonicalClient
 *
 * Metáfora: é a “língua única” que todos os arquivos viram,
 * antes de entrar no ClientDB.
 */
...
export const CanonicalClientSchema = z.object({
  ...
  // Internacional: guardar texto livre agora, estruturar depois.
  addressFreeform: z.string().optional(),

  source: CanonicalSourceSchema,

  // Confiança por campo (0..1). Ex.: { fullName: 0.9, phone: 1.0 }
  confidence: z.record(z.number().min(0).max(1)).default({}),

  // Opcional: guardar amostra original (debug/auditoria leve).
