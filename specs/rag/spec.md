...
- **Código:** [src/modules/rag/](src/modules/rag/) (tipos, `fonte`, `agrupar_por_turno`, `embed_index`, `guardar`, `embed_consulta`, `buscar`, `orquestrador_indexacao_rag`, `orquestrador_consulta_rag`).
...
- Implementado como uma interface lógica `VectorIndex` (ex.: implementação `VectorIndexLocal`), responsável por:
...
- O detalhe de armazenamento físico (arquivo local, SQLite, etc.) é interno à implementação do `VectorIndex` e não faz parte deste contrato.
